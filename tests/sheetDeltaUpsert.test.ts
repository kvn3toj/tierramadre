...
describe('planRowPatch — diff-skip', () => {
  ...
  it('ignores the still-excluded preponderancia column', () => {
    // preponderancia stays Convex-owned; costoBaseCOP became sheet-owned on
    // 2026-07-24 and is covered by its own pull test in sheetPullMaps.test.ts.
  ...
  it('pulls the now sheet-owned costoBaseCOP column', () => {
    const plan = planRowPatch('inventory', doc({ costoBaseCOP: 100 }), {
      costoBaseCOP: '999999',
    });
    expect(plan.action).toBe('patch');
    expect(plan.patch).toEqual({ costoBaseCOP: 999999 });
  });

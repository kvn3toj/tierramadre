...
**How it must feel:** _Una joya en calma._ Gallery-quiet, editorially confident, never busy. Calm is a feature: no flicker, no scroll fights, no layout jumps, no color noise.
...
**One saturated color in the entire product: the emerald.** Everything else is grayscale. If a screen contains a second saturated hue, it is either a semantic status color doing real work, or a bug.

The light-mode neutrals are **achromatic** (chroma 0). They previously carried a faint emerald tint, which was retired for one reason: the tint was the same hue as the product. A green wall behind green stones flattens them; a neutral wall makes them read greener by simultaneous contrast. The background's job is to disappear so the pieces do the talking. Depth comes from **lightness alone** — never from hue.

### 1.2 Token map (authoritative — from `tokens/quiet-emerald.ts`)

| Token                | Light         | Dark          | Role                                                  |
| -------------------- | ------------- | ------------- | ----------------------------------------------------- |
| `--tm-bg`            | `#EFEFEF`     | `#0E1110`     | App background (the gallery wall)                     |
| `--tm-surface`       | `#FFFFFF`     | `#15191A`     | Card / raised surface                                 |
| `--tm-well`          | `#E8E8E8`     | `#1B1F1F`     | Image well behind a piece — the vitrine floor         |
| `--tm-border`        | `#DCDCDC`     | `#272C2B`     | 1px component borders                                 |
| `--tm-hairline`      | `#E6E6E6`     | `#222726`     | 1px row dividers / section rules                      |
...
- Structure comes from `--tm-border` / `--tm-hairline` and _surface steps_ (`bg → surface → well`), not from shadows.
...
3. **Titles follow depth**: `large` nav mode for family roots (editorial serif title), `compact` for descendants. Detail screens title with the _piece name_, not the section name.
...
4. **Scroll position is memory.** Back/forward restores the _actual scrolled element_'s offset (main, or the virtual grid via `scrollMemory` keyed by route + filters). New forward navigation resets to top. A user returning from a product always lands on the row they left.
...
_DS v3 — designed against the live audit of 2026-07-17 (`DESIGN-SYSTEM-AUDIT.md`, `SCROLL-UIUX-AUDIT-2026-07-08.md`). Rules 5.1–5.4 codify and extend the shell architecture already proven in Fotosíntesis/Atelier to the whole product._

...
| Certificate QR | `QuotationCertificate.tsx:559-581`    | `quotationData.selectedProducts` (existing field)                        | via existing `getQrCodeUrl(products)` from `src/components/cotizacion/utils.ts` — unchanged logic, just newly consumed here |
...
- `QuotationCertificate`: `getQrCodeUrl()` already has its own fallback path (empty products → treasure-browser link), so no new edge case here.

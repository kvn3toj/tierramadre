...
/**
 * Build the `updateGemaFields` patch that persists a lot item's catalog
 * visibility from the LoteResumenPage / EditItemDrawer.
 *
 * PRICE REFACTOR (2026-07-21): the per-item price tiers were removed — the price
 * is now DERIVED in Convex (precioFinalCOP = costoBaseCOP × 2.6), so the UI no
 * longer sends any price here. This patch carries ONLY the publish/reserva
 * toggle, which the save handlers must never drop.
 */
export function buildItemPricingPatch(mostrarEnCatalogo: boolean): {
  mostrarEnCatalogo: boolean;
} {
  return { mostrarEnCatalogo };
}

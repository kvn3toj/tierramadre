...
// providers/clients) is now an idToken-gated Convex `action`, not a plain
// `mutation` — see convex/_lib/authz.ts. Inject the caller's fresh Google ID
// token here, once, instead of threading it through all ~25 call sites.
function runMutation<T>(
  ...
  const argsWithToken = { ...(args as Record<string, unknown>), idToken };

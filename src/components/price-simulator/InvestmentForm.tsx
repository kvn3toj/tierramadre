...
import { studioColors, fontWeights } from '../../design-system';
...
export const CaratWeightInput: React.FC<CaratWeightInputProps> = ({
  caratWeight,
  setCaratWeight,
}) => {
  const { t } = useLanguage();
      ...
      <TextField
        ...
        sx={{
          '& .MuiOutlinedInput-root': {
            ...
            '&:hover fieldset': {
              borderColor: alpha(studioColors.emerald, 0.4),
            },
    ...
    <>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {investments.map((item) => (
          <Box key={item.id}>
            ...
            >
              <Box sx={{ color: studioColors.textSecondary }}>
                {getInvestmentIcon(item.id)}
              </Box>
              <Typography
                variant="body2"
                sx={{
            ...
            <TextField
              ...
              value={item.value || ''}
              onChange={(e) =>
                updateInvestment(item.id, Number(e.target.value) || 0)
              }
      ...
      <Box sx={{ mt: 3 }}>
        ...
        >
          <Typography
            variant="body2"
            sx={{ fontWeight: 600, color: studioColors.textSecondary }}
          >
            Costos adicionales
          </Typography>
          {showAdvanced ? (
            <ChevronUp size={18} color={studioColors.textSecondary} />
          ) : (
            <ChevronDown size={18} color={studioColors.textSecondary} />
          )}
        </Box>
            ...
            {customItems.map((item, index) => (
              <Box key={index} sx={{ display: 'flex', gap: 1.5 }}>
                <TextField
                  ...
                  value={item.label}
                  onChange={(e) =>
                    updateCustomItem(index, 'label', e.target.value)
                  }

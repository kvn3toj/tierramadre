/**
 * useInventoryMedia Hook
 *
 * Manages media storage for inventory items:
...
// Storage keys
const LEGACY_STORAGE_KEY = 'tierramadre-inventory-media';
...
export interface UseInventoryMediaReturn {
  // Legacy single media (backwards compatibility)
  legacyMedia: LegacyInventoryMedia;
...
function loadLegacyMedia(): LegacyInventoryMedia {
  try {
...
function saveLegacyMediaToStorage(media: LegacyInventoryMedia): void {
  try {
...
export function useInventoryMedia(): UseInventoryMediaReturn {
...
export default useInventoryMedia;

...
export const useInvitation = (): UseInvitationReturn => {
  ...
  const [lastInvitation, setLastInvitation] = useState<InvitationData | null>(
    null,
  );
  ...
  const validateInvitation = useCallback(
    async (shortCode: string): Promise<ValidationResult> => {
      setIsValidating(true);
      setError(null);

      try {
        const response = await fetch(
          `/api/invitations?action=validate&code=${encodeURIComponent(shortCode)}`,
        );
        const data = await response.json();
  ...
  const verifyPin = useCallback(
    async (shortCode: string, pin: string): Promise<PinVerificationResult> => {
      setIsVerifyingPin(true);
      setError(null);

      try {
        const storedToken =
          localStorage.getItem(INVITATION_STORAGE_KEYS.DEVICE_TOKEN) ||
          undefined;
  ...
  const registerGuest = useCallback(
    async (registration: GuestRegistration): Promise<boolean> => {

...
async function getProviderFromSheet() {
  if (!isGoogleConfigured()) {
    console.warn(
      "[Email] Google not configured - cannot lookup provider email",
    );
    return null;
  }
...
export default withApiHandler(
  async (req, res) => {
    ...
    if (toAdmins) {
      ...
      recipients = to
        ? [...(Array.isArray(to) ? to : [to]), ...adminEmails]
        : adminEmails;
    }

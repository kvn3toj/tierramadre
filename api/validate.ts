...
async function validateUser(
  ...
  for (const row of dataRows) {
    ...
    const userEmail =
      emailIndex !== -1 ? String(row[emailIndex] || '').toLowerCase().trim() : '';

    if (userEmail === normalizedEmail) {
      const name = nameColumnIndex !== -1 ? row[nameColumnIndex] : '';
      const role = roleIndex !== -1 ? String(row[roleIndex] || 'Asesor').trim() : 'Asesor';
 ...
 */
async function listProviders(sheets: Sheets, sheetNames: string[]): Promise<ProviderRow[]> {
  ...
  return rows
    ...
    .map((row) => ({
      ...
      contactPerson: contactoIndex !== -1 ? String(row[contactoIndex] ?? '') : '',
      whatsapp: whatsappIndex !== -1 ? String(row[whatsappIndex] ?? '') : '',
      specialty: especialidadIndex !== -1 ? String(row[especialidadIndex] ?? '') : '',
      status: estadoIndex !== -1 ? String(row[estadoIndex] ?? 'ACTIVO') : 'ACTIVO',
...
export default withApiHandler(
  async (req: VercelRequest, res: VercelResponse, context: Record<string, unknown>) => {
    const sheets = context.sheets as Sheets;
    const action = (req.query.action as string) || req.body?.action || 'validate';
